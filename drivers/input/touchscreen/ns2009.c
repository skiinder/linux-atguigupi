...
static int ns2009_ts_report(struct ns2009_data *data)
{
	...
	if (z1 >= NS2009_PEN_UP_Z1_ERR) {
		...
		dev_info(&data->input->dev, "x: %d, y: %d\n", x, y);
...
{
	struct ns2009_data *data = input_dev->dev.driver_data;
	int ret;
...
static int ns2009_ts_request_polled_input_dev(struct ns2009_data *data)
{
	...
	input_dev->dev.driver_data = data;

	error = input_setup_polling(input_dev, ns2009_ts_poll);
	if (error != 0) {
		dev_err(dev, "Failed to allocate polling input device\n");
		return -error;
	}
